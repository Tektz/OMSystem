...
if(isset($_POST["category"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		...
		while($row = mysqli_fetch_array($run_query)){
            ...
            $sql = "SELECT COUNT(*) AS count_items FROM product WHERE cat_id=$i";
            $query = mysqli_query($con,$sql);
...
if(isset($_POST["brand"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		...
		while($row = mysqli_fetch_array($run_query)){
            ...
            $sql = "SELECT COUNT(*) AS count_items FROM product WHERE brand_id=$i";
            $query = mysqli_query($con,$sql);
            $row = mysqli_fetch_array($query);
            $count=$row["count_items"];
            $i++;
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		...
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM product a,categories b WHERE a.brand_id = '$id' AND a.cat_id=b.cat_id";
	}else {
        
		$keyword = $_POST["keyword"];
        header('Location:store.php');
		$sql = "SELECT * FROM product a,categories b WHERE a.cat_id=b.cat_id AND product_keywords LIKE '%$keyword%'";  
	}
...
//new cart
	if(!empty($_POST["quantity"])) {
		$productByCode = $con->runQuery("SELECT * FROM product WHERE product_id='" . $_GET["code"] . "'");
		$itemArray = array($productByCode[0]["code"]=>array('product_name'=>$productByCode[0]["name"], 'product_id'=>$productByCode[0]["code"], 'quantity'=>$_POST["quantity"], 'product_price'=>$productByCode[0]["price"], 'product_image'=>$productByCode[0]["image"]));
...
if (isset($_POST["count_item"])) {
	...
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE buyer_id = $_SESSION[uid]";
	}else{
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE ip_add = '$ip_add' AND buyer_id < 0";
	}
...
if (isset($_POST["Common"])) {
	...
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT a.product_id,a.product_name,a.product_price,a.product_image,b.cart_id,b.qty FROM product a,cart b WHERE a.product_id=b.p_id AND b.buyer_id='$_SESSION[uid]'";
	}else{
		$sql = "SELECT a.product_id,a.product_name,a.product_price,a.product_image,b.cart_id,b.qty FROM product a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.buyer_id < 0";
	}
	$query = mysqli_query($con,$sql);
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="checkout.php" method="post">
							...
							<input type="hidden" name="business" value="sales@oms.com">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/oms/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/oms/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/oms/cancel.php"/>
									<input type="hidden" name="currency_code" value="TSH"/>
...
if (isset($_POST["removeItemFromCart"])) {
	$remove_id = $_POST["rid"];
	if (isset($_SESSION["uid"])) {
		$sql = "DELETE FROM cart WHERE p_id = '$remove_id' AND buyer_id = '$_SESSION[uid]'";
	}else{
...
if (isset($_POST["updateCartItem"])) {
	...
	$qty = $_POST["qty"];
	if (isset($_SESSION["uid"])) {
		$sql = "UPDATE cart SET qty='$qty' WHERE p_id = '$update_id' AND buyer_id = '$_SESSION[uid]'";
	}else{
